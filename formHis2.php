<?php
	...
	// history
	$sqlv="SELECT * FROM `dep_project`.`formhistory` as F where F.Status='Post Operation' or F.Status='Completed';";
	$resv=$con->query($sqlv);
	$num_t=$resv->num_rows;
	$fidarr=[];
	$pidarr=[];
	...
	if(isset($_POST['buttons1'])){
        ...
        $SortStatus=false;
        $SortPatientId=false;
		$sqlh1="SELECT * FROM `dep_project`.`formhistory` as F where F.Status='Post Operation' or F.Status='Completed' ORDER BY FormID";
    ...
    if(isset($_POST['buttons2'])){
        ...
        $SortStatus=false;
        $SortPatientId=false;
		$sqlh2="SELECT * FROM `dep_project`.`formhistory` as F where F.Status='Post Operation' or F.Status='Completed' ORDER BY DateRequired";
    ...
    if(isset($_POST['buttons3'])){
        ...
        $SortStatus=false;
        $SortPatientId=false;
		$sqlh3="SELECT * FROM `dep_project`.`formhistory` as F where F.Status='Post Operation' or F.Status='Completed'  ORDER BY DateCreated";
    ...
    if(isset($_POST['buttons4'])){
        ...
        $SortStatus=true;
        $SortPatientId=false;
		$sqlh4="SELECT * FROM `dep_project`.`formhistory` as F where F.Status='Post Operation' or F.Status='Completed' ORDER BY  Status";
    ...
    if(isset($_POST['buttons5'])){
        ...
        $SortStatus=false;
        $SortPatientId=true;
		$sqlh5="SELECT * FROM `dep_project`.`formhistory` as F where F.Status='Post Operation' or F.Status='Completed' ORDER BY  PatientID";
...
<span class="navbar-text">
	...
	<a class="btn btn-dark" style = 'background-color:rgb(195 15 118)' href="formHis2.php">Completed Forms</a>
...
<div class="container">
	<div id="bh">
		<table class="table">
  		<tr scope="row">
    			<th>S.No.</th>
                <th><form action="formHis2.php" method="post">
  	...
  	<input type="submit" name="buttons4" class="btn btn-success text-center" style = "background-color:DodgerBlue"  value="Status">
		...
		<?php   
  		...
  		<tr scope="row">
  			...
  			<td><a href="record_file.php?formid=<?php echo $fidarr[$i]; ?>" target="_blank" rel="noopener noreferrer"><?php echo $fidarr[$i];?></a></td>
