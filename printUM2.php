...
<body>
<form action="printUM2.php" method="post">
    ...
    <table class="table">
        <tr scope="row">
          ...
          <?php
            ...
            $sql1 = "SELECT * from `dep_project`.seca_items";
            $result1 = $conn->query($sql1);
            $sql2 = "SELECT * from `dep_project`.secb_items";
            $result2 = $conn->query($sql2);
            $sql3 = "SELECT * from `dep_project`.secc_items";
            $result3 = $conn->query($sql3);
            $sql4 = "SELECT * from `dep_project`.secd_items";
            $result4 = $conn->query($sql4);
            $sql5 = "SELECT * from `dep_project`.sece_items";
            ...
            if($num_rows <= 0){
                ...
                ?>
                            ...
                            <div class="col-1" style="margin-top: 5px;">
                                <div style="overflow:hidden"></div><input style="width:50px" type="number" step="1" min="0" max="1000" value="<?php echo $arre_qty_req[$index-1];?>" name = "<?php echo "q".$kjh;?>"
                                <?php if(isset($_POST['buttonSub1'])) array_push($arr_qty_supplied, getQtySupplied('q'.$kjh)); ?>></div>
                            </td>
                            <td>
                            <div class="col-2" style="margin-top: 5px;">
                                <div style="overflow:hidden"></div><input type="text" style="width:200px" value = "" name = "<?php echo "r".$kjh; ?>"
                                <?php if(isset($_POST['buttonSub1'])) array_push($arr_remarks, getRemark('r'.$kjh)); $kjh += 1; ?>></div>
                            </div> 
                            </td>
...
<div class="container">
                <div class="row">
                    <div class="col">
                        <div class="text-right"></div><button class="btn btn-success text-center" type="submit" name="buttonSub1" onclick="location.href='formHis_um.php'">Submit</button></div>
                </div>
</div>
</form>
...
if(isset($_POST['buttonSub1'])){
    ...
    $sql_t = "TRUNCATE TABLE `dep_project`.`$formId1`";
    $res_t = $conn->query($sql_t);
    
    //if(!$res_t) echo $conn->error;

    $num_items = sizeof($arr_remarks);
    
    for($w=0;$w<$num_items;$w++){
        $w1=$w+1;
        //echo print_r($arr_qty_supplied);
        $sql_c = "INSERT INTO `dep_project`.`$formId1`(`srno`,`checked`,`qty`, `remark`) VALUES ('$w1','$arr_checked[$w]','$arr_qty_supplied[$w]','$arr_remarks[$w]');";
        $res = $conn->query($sql_c);
        if(!$res) echo $conn->error;
    }
    //echo "<h2 id='succ'>Successfully completed.</h2>";
    echo '<script>alert("Changes saved Successfully!.. You can edit the changes later using \"Change Supplied Items\" for a formID")</script>';
    header('refresh:0.5;url=formHis_um.php');
    //header('Location:http://localhost/dep_Inventory1/formHis_um.php');
