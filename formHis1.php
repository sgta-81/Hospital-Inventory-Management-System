<?php
	...
	$postOpQuery=false;
    if(isset($_SESSION['postOpRows'])){
        for($xx=1;$xx<=$_SESSION['postOpRows'];$xx++){
            $recvi="verif".$xx;
	...
	// history
	$sqlv="SELECT * FROM `dep_project`.`formhistory` as F where F.Status='Stock Rcvd' or F.Status='Stock Rcvd from Unit Manager' or F.Status='Stock Verified and fwd' or F.status = 'Dispatched to Pharma' or F.status = 'Issue in rcvd items - reported to UM';";
	$resv=$con->query($sqlv);
	$num_t=$resv->num_rows;
	$fidarr=[];
	$pidarr=[];
	...
	if(isset($_POST['buttons1'])){
        ...
        $SortStatus=false;
        $SortPatientId=false;
		$sqlh1="SELECT * FROM `dep_project`.`formhistory` as F where F.Status='Stock Rcvd' or F.Status='Stock Rcvd from Unit Manager' or F.Status='Stock Verified and fwd' or F.status = 'Dispatched to Pharma' or F.status = 'Issue in rcvd items - reported to UM' ORDER BY FormID";
    ...
    if(isset($_POST['buttons2'])){
        ...
        $SortStatus=false;
        $SortPatientId=false;
		$sqlh2="SELECT * FROM `dep_project`.`formhistory` as F where F.Status='Stock Rcvd' or F.Status='Stock Rcvd from Unit Manager' or F.Status='Stock Verified and fwd' or F.status = 'Dispatched to Pharma' or F.status = 'Issue in rcvd items - reported to UM' ORDER BY DateRequired";
    ...
    if(isset($_POST['buttons3'])){
        ...
        $SortStatus=false;
        $SortPatientId=false;
		$sqlh3="SELECT * FROM `dep_project`.`formhistory` as F where F.Status='Stock Rcvd' or F.Status='Stock Rcvd from Unit Manager' or F.Status='Stock Verified and fwd' or F.status = 'Dispatched to Pharma' or F.status = 'Issue in rcvd items - reported to UM' ORDER BY DateCreated";
    ...
    if(isset($_POST['buttons4'])){
        ...
        $SortStatus=true;
        $SortPatientId=false;
		$sqlh4="SELECT * FROM `dep_project`.`formhistory` as F where F.Status='Stock Rcvd' or F.Status='Stock Rcvd from Unit Manager' or F.Status='Stock Verified and fwd' or F.status = 'Dispatched to Pharma' or F.status = 'Issue in rcvd items - reported to UM' ORDER BY  Status";
    ...
    if(isset($_POST['buttons5'])){
        ...
        $SortStatus=false;
        $SortPatientId=true;
		$sqlh5="SELECT * FROM `dep_project`.`formhistory` as F where F.Status='Stock Rcvd' or F.Status='Stock Rcvd from Unit Manager' or F.Status = 'Stock Verified and fwd' or F.status = 'Dispatched to Pharma' or F.status = 'Issue in rcvd items - reported to UM' ORDER BY  PatientID";
...
<span class="navbar-text">
	...
	<a class="btn btn-dark" style = 'background-color:#00BFFF' href="formHis.php">View Incomplete Forms</a>
    <a class="btn btn-dark" style = 'background-color:rgb(195 15 118)' href="formHis1.php">View Dispatched Forms</a>
...
<div class="container">
	<div id="bh">
		<table class="table">
  		<tr scope="row">
    			<th>S.No.</th>
                <th><form action="formHis1.php" method="post">
  	...
  	<input type="submit" name="buttons4" class="btn btn-success text-center" style = "background-color:DodgerBlue"  value="Status">
		...
		<?php   
  		...
  		<tr scope="row">
  			...
  			<td><a href="printUM1.php?formid=<?php echo $fidarr[$i]; ?>" target="_blank" rel="noopener noreferrer"><?php echo $fidarr[$i];?></td>
			...
			if ($statusarr[$i] == 'Approved by Consultant'){ ?>
				...
				<?php echo $statusarr[$i];?></td>
			<?php } 
			if ($statusarr[$i] == 'Issue in rcvd items - reported to UM'){ ?>
				<td style="background-color: #FFA07A ;border: 1px solid black;">
				<?php echo $statusarr[$i];?></td>
			<?php }  
            if($statusarr[$i]=='Stock Rcvd from Unit Manager'){
            ?>  	

			<td><a href="printUM3_Nurse.php?formid=<?php echo $fidarr[$i]; ?>">Verify Stock</a></td>    
			<!--td><a href="editForm.php?formid=<?php echo $fidarr[$i]; ?>" target="_blank" rel="noopener noreferrer">Verify Stock</a></td-->
			<?php 
            }
            if ($statusarr[$i]=='Stock Verified and fwd'){ ?>
				<td><div class="text-right"></div><a class="btn btn-success text-center" style = "color:white;background-color:DodgerBlue;width:185px;" 
                    href="work_reception.php?formid=<?php echo $fidarr[$i]; ?>" name = "<?php echo 'verif'.$i;?>">Enter consumed items</a></td>
		  <?php } ?>
		</tr>
  		<?php }}	 ?>
        </table>
