...
<body>
<form action="printUM3.php" method="post">  
...
<div class="container">
                <div class="row">
                    <div class="col">
                    <div class="text-right"></div><a href = 'editFormUM.php?formid=<?php echo $formid; ?>' class="btn btn-success text-center" style = "background-color:black;color:white;">Edit this form</a></div>
                    <div class="col">
                    <div class="text-right"></div><button onclick="location.href = '';" class="btn btn-success text-center" style = "background-color:DodgerBlue" type="submit" name="send">Send items to Hospital</button></div>
                </div>
</div>
...
if(isset($_POST['send'])){
    updateFormStatus($formid,"Stock Rcvd from Unit Manager",$conn); //nurse staff view
    updateFormStatus3($formid,"Stock Verification Pending",$conn); //um view
    
    echo '<script>alert("Items sent Successfully!.. You will be notified once the Nurse verifies the items received, or reports an issue")</script>';
